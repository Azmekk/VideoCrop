import { Button, Checkbox, Divider, Dropdown, Input, InputNumber, Radio, Space, Splitter, Switch, type MenuProps } from "antd";
...
function CropSegment(props: CompressSegmentProps) {
    ...
    const determinePreset = () => {
        ...
        else if (selectedCodec == "libsvtav1") {
    ...
    return (
        <div style={{ opacity: props.disabled ? 0.5 : 1, pointerEvents: props.disabled ? 'none' : 'auto', }}>
            <div>
                ...
                <div className={segmentEnabled ? "" : "disabled"} style={{ gap: '10px', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div>
                        {selectedQualityOption == 1 ?
                            ...
                            <InputNumber addonAfter={"kbps"} type="number" placeholder="Bitrate" defaultValue={5500} onChange={(e) => setSelectedBitrate(e ?? 0)} />
                        }
                    </div>
                </div>
            </div>
